...
def shoot_number_to_accession_id(accession_number, shoot_number):
    ...
    if accession_number and shoot_number:
        ...
        raise ValueError(f"misssing accession or shoot number - accession: '{accession_number}' shoot: '{shoot_number}'")
...
def generate_metadata_csv(csvfile, accession_id):
    ...
    """
    writer = csv.DictWriter(csvfile, fieldnames=["filename", "collection_reference", "accession_number"])
    writer.writeheader()
    writer.writerow(
        {"filename": "/objects", "collection_reference": "WT", "accession_number": accession_id}
    )
    return csvfile
...
def move_files_to_objects_folder(source_files, target_directory):
    ...
    for file_abspath in source_files:
        shutil.move(
            file_abspath,
            os.path.join(target_directory, "objects")
        )
...
def files_in_folder(source_directory):
    ...
    """
    return [filename for filename in os.listdir(source_directory) if not discard_file(filename)]
...
def create_born_digital_zips(source_directory, target_directory, accession_number, shoot_number, max_batch_size):
    filenames = files_in_folder(source_directory)

    if len(filenames) == 0:
        raise FileNotFoundError("Attempt to build born digital accession zip from empty folder")
    ...
    if len(filenames) > max_batch_size:
        ...
        for ix, batch in enumerate(batches, 1):
            ...
            yield create_born_digital_zip(full_paths(source_directory, batch), os.path.join(target_directory, accession_id), accession_id)
    else:
        accession_id = shoot_number_to_accession_id(accession_number, shoot_number)
        yield create_born_digital_zip(full_paths(source_directory, filenames), os.path.join(target_directory, accession_id), accession_id)
